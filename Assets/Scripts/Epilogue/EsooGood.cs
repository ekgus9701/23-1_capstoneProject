...
public class EsooGood : MonoBehaviour
{
   ...
   IEnumerator TextPractice(){
    ...
    yield return StartCoroutine(NormalChat("윤이수","오, "+me+"... 너 좀 쓴다? "));
    ...
    boyTag.gameObject.SetActive(true);
    boyImage[2].gameObject.SetActive(true);
    yield return StartCoroutine(NormalChat("윤이수","응. 너 이러다 진짜 상 타겠는데? 게다가 "+me+" 어휘력도 엄청 좋은 것 같아."));
    yield return StartCoroutine(NormalChat("윤이수","또… 나랑 있었던 일로 이렇게 소설까지 써줬다고 생각하니까 좀 기쁜데…"));
    yield return StartCoroutine(NormalChat("윤이수","야, 너 서울 가도 계속 연락해도 되냐?"));
