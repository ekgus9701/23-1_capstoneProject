...
public class HJNormal : MonoBehaviour
{
   ...
   IEnumerator TextPractice(){
    ...
    yield return StartCoroutine(NormalChat("소현재","오, 글 좋은데?"));
    ...
    boyTag.gameObject.SetActive(true);
    boyImage[3].gameObject.SetActive(true);
    yield return StartCoroutine(NormalChat("소현재","응. 은근 흥미로운데? 게다가 "+me+" 어휘력도 좀 좋아졌다?"));
    yield return StartCoroutine(NormalChat("소현재","또… 나랑 있었던 일로 이렇게 소설까지 써줬다니까 영광인걸?"));
    yield return StartCoroutine(NormalChat("소현재","서울 가면 또 맛있는 거 사줘~"));
    ...
    yield return StartCoroutine(NormalChat("","그리고 소현재에게도 종종 응원 연락이 온다."));
